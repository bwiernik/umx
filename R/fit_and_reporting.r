...
#'
#' Note: This is not a full-fat fit reporter. It is not robust across multi-group designs or
...
umxFitIndices <- function(model, refModels = suppressMessages(mxRefModels(model, run = TRUE))) {
  ...
  Chi.df       = Chi/df
  indep.lik    = modelSummary$IndependenceLikelihood
  indep.df     = modelSummary$independenceDoF
  sat.lik      = modelSummary$SaturatedLikelihood
  sat.df       = modelSummary$saturatedDoF
  indep.chi    = indep.lik - sat.lik
  ...
  if(!all(is.na(observed.means))) {
    estimate.means = mxGetExpected(model, "means")
  } else {
    estimate.means = NULL
  }
  Id.manifest =  diag(N.manifest)
  omxFitStats =  OpenMx:::fitStatistics(model, 
                                        refModels$Independence, 
                                        list(Minus2LogLikelihood = deviance,
                                             SaturatedLikelihood = sat.lik,
                                             IndependenceLikelihood = ))
  omxFitStats =  OpenMx:::computeFitStatistics(deviance, df, 
                                               Chi, df, N, 
                                               indep.lik, indep.df, 
                                               sat.lik, sat.df)
  RMSEA          =  modelSummary$RMSEA
  RMSEACI        =  modelSummary$RMSEACI
  RMSEAClose     =  modelSummary$RMSEACLOSE
  RMSEASquared   =  modelSummary$RMSEASquared
  RMSEANull      =  modelSummary$RMSEANull
  F0             =  RMSEASquared * df
  NCP            =  F0 * N
  NCPCI          =  RMSEACI^2 * N * df
  GH             =  N.manifest / (N.manifest + 2 * F0)
  Mc = NCI = MFI =  exp(-0.5 * F0)
  
  
  TLI = NNFI     =  omxFitStats$TLI
  CFI            =  omxFitStats$CFI
  
  NFI            =  (indep.chi - Chi) / indep.chi
  RFI            =  1 - (Chi / (df - sat.df)) / (indep.chi / (indep.df - sat.df))
  IFI = BL89     =  (indep.chi - Chi) / (indep.chi - (df - sat.df))
  PRATIO         =  (df - sat.df) / (indep.df - sat.df)
  PCFI           =  PRATIO * CFI
  PNFI           =  PRATIO * NFI
  RNI            =  ((indep.chi - (indep.df - sat.df)) - Chi + (df - sat.df)) / (indep.chi - (indep.df - sat.df))
  ...
  AGFI =  1 - (q / (df - sat.df)) * (1 - GFI)
  PGFI =  GFI * (df - sat.df) / q
  
  # Information-based indices
  infoCrit = modelSummary$informationCriteria
  AIC.Mx   = AIC.df = infoCrit[1,1]
  AIC.par  = infoCrit[1,2]
  AICc     = infoCrit[1,3]
  
  BIC.Mx   = BIC.df = infoCrit[2,1]
  BIC.par  = infoCrit[2,2]
  sBIC     = infoCrit[2,2]
  
  CAIC     = AIC.par - 2 * N.parms + N.parms * (log(N) + 1)
  ECVI     = AIC.par / N
  BCC      = AIC.par - 2 * N.parms + 2 * N.parms/(N - N.manifest - 2)
  MECVI    = BCC.par / N
  
  # Residual based indices
  # TODO: Add unbiased standardized estimators
  ...
  indep.RMSEA  = sqrt(max((indep.chi / indep.df - 1) / N, 0))
...
print.umxFitIndices <- function(x, digits = max(1L, getOption("digits") - 3L), ...) {
  ...
  cat('RNI (Relative noncentrality index):', round(x$RNI,       digits), '\n')
  ...
  cat('==================================\n')
  cat('(These indices are strongly discouraged due to numerous problems. It is recommended that you do not report them.)\n')
